...
Si la suite omite las pruebas HTTP porque no detecta la IP de la computadora, especificar la IP real asignada en la red (no `localhost` ni `127.0.0.1`). Por ejemplo:

```bash
TIC_HOST=192.168.1.30 bash tests/run_tests.sh
```

**Sustituir `192.168.1.30` por la IP de la computadora donde corre el servidor.** Se puede consultar con `hostname -I`. Este comando ejecuta **toda la suite**, incluidas las pruebas HTTP; no corrige otros posibles errores de ejecución.
...
|---|---|---|---|---|
| FS-01 | FileSystem: crear bodega | `crearBodega("Test")` devuelve un identificador positivo. | Ejecutar `filesystem_test.cc` (comando de abajo). | **Aprobada**, dentro de la prueba funcional C++. |
| FS-02 | FileSystem: búsqueda inexistente | `buscarBodega(-1)` no encuentra ninguna bodega y devuelve `nullptr`. | Mismo programa C++. | **Aprobada**. |
| FS-03 | FileSystem: agregar productos | Agregar 6 productos y comprobar que reciben los ID esperados (1 a 6). | Mismo programa C++. | **Aprobada**. |
| FS-04 | FileSystem: listado | `listarTodosLosProductos()` muestra los 6 productos agregados. | Mismo programa C++. | **Aprobada**. |
| FS-05 | FileSystem: crecimiento | Al agregar 6 productos, la bodega pasa a usar al menos 2 bloques (`cantidadBloques >= 2`). | Mismo programa C++. | **Aprobada**. |
| FS-06 | FileSystem: actualizar cantidad | Cambiar a 4 unidades el primer producto y leer de nuevo la cantidad para comprobar el cambio. | Mismo programa C++. | **Aprobada**. |
| FS-07 | FileSystem: actualizar precio | Cambiar el precio del segundo producto a `12.75` y comprobar el valor leído. | Mismo programa C++. | **Aprobada**. |
| FS-08 | FileSystem: persistencia de cambios | Destruir y volver a crear el objeto `FileSystem` y comprobar que siguen los 6 productos, la cantidad 4 y el precio 12.75. | Mismo programa C++. | **Aprobada**. |
| FS-09 | FileSystem: búsqueda por categoría | `buscarPorCategoria("Pruebas")` devuelve los 6 productos de esa categoría. | Mismo programa C++. | **Aprobada**. |
| FS-10 | FileSystem: eliminación | Eliminar el sexto producto y comprobar que la lista baja de 6 a 5. | Mismo programa C++. | **Aprobada**. |
| FS-11 | FileSystem: persistencia de eliminación | Volver a crear el objeto `FileSystem` y comprobar que quedan 5 productos. | Mismo programa C++. | **Aprobada**. |
...
**Nota:** `FS-01` a `FS-11` son las verificaciones que realiza **un único programa de prueba C++** (`filesystem_test.cc`); no son 11 pruebas independientes. Este programa no verifica el límite de 256 bytes ni el uso de un archivo único.

Si el comando de las pruebas HTTP de la tabla las omite porque no detecta una IP válida, se puede ejecutar **solo HTTP** especificando la IP del servidor:

```bash
TIC_HOST=192.168.1.30 python3 -m unittest discover -s tests -p 'test_http.py' -v
```

La IP `192.168.1.30` es solo un ejemplo: reemplazarla por la propia. También se puede utilizar `TIC_HOST=192.168.1.30 bash tests/run_tests.sh` para ejecutar todo con esa IP.
